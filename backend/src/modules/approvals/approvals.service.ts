...
import { SbtMetadataService } from '../sbt-metadata/sbt-metadata.service';
import { SbtTokensService } from '../sbt-tokens/sbt-tokens.service';
...
export class ApprovalsService {
  ...
  async listPendingForManager(user: AuthUser) {
  ...
  async getSigningPayload(requestId: string, user: AuthUser) {
    this.assertManager(user);
  ...
  async signAndMint(payload: SignApprovalDto, user: AuthUser) {
    this.assertManager(user);
  ...
  private assertApprovingManager(context: ApprovalContext, user: AuthUser) {
    if (!this.isCurrentManager(context.manager, user)) {
  ...
  private isCurrentManager(manager: UserRecord, user: AuthUser) {
    return (
      manager.id === user.sub ||
