...
> **선행 조건**: Supabase DB 구축 + Edge Functions 배포 완료 상태  
> **비용**: Railway Hobby $5/월 (idle 기준 월 ~$3 예상, $5 크레딧 내 운영 가능)

1. railway.app 가입 → GitHub 계정 연동
2. New Project → Deploy from GitHub repo → NestJS 레포 선택
3. Railway Variables에 환경변수 입력

   | 키 | 출처 |
   |----|------|
   | `DATABASE_URL` | Supabase Settings → Database → Connection string (SSL 모드) |
   | `SUPABASE_URL` | Supabase Settings → API → Project URL |
   | `SUPABASE_SERVICE_ROLE_KEY` | Supabase Settings → API |
   | `JWT_SECRET` | C가 공유한 JWT 명세 |
   | `PLATFORM_PRIVATE_KEY` | sig2 서명용 플랫폼 ETH 개인키 |

4. Settings → Build Command: `npm run build` / Start Command: `npm run start:prod`
5. Deploy → 빌드 완료 후 Railway 공개 URL 발급
6. 발급된 URL → Supabase Secrets에 `NESTJS_SERVER_URL` 등록
   ```bash
   supabase secrets set NESTJS_SERVER_URL=https://프로젝트명.up.railway.app
   ```
7. 팀 전체에 URL 공유 (A: 모바일 앱 BASE_URL 환경변수 설정)

> **재배포**: main 브랜치 push 시 Railway가 자동으로 재빌드·배포
...
*서버 구조: B-1·B-2는 단일 NestJS 서버를 공유. B-1이 `CommonModule(@Global())`을 구현 → AppModule에서 B-1 모듈 먼저 import 후 B-2 모듈 import. B-2는 Guard 재구현 없이 사용 가능. 개발 중에는 각자 로컬(localhost)에서 실행. 시연 준비 시 PM이 main 브랜치 머지 완료 후 PM이 Railway에 단일 서버 배포 → 공개 URL을 팀 전체에 공유.*
...
| PM (자체) | NestJS 서버 배포 URL | PM이 직접 Railway 배포 → 공개 서버 URL 팀 전체 공유. Supabase Secrets에 NESTJS_SERVER_URL 등록 | main 브랜치 머지 완료 후 |
...
| **NestJS 서버 배포 URL** | Railway 배포 완료 후 공개 서버 BASE_URL 전달 → A가 모바일 앱 환경변수에 설정하여 B-1·B-2 전체 API 연동 | PM 머지 + PM 배포 완료 후 |
...
- ✅ **[PM]** NestJS 서버 Railway 배포 → 공개 URL 팀 전체 공유 (main 머지 완료 후, Phase 2 시작 전)
...
- 🔜 **랭킹 산출** (전체 회원들의 실시간 레벨 조회 후 특정 회원의 현 위치 정보 제공. e.g. x님은 현재 상위 10%입니다.)
